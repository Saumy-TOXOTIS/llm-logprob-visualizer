...
export function AnswerSpaceExplorer() {
  ...
  return (
    <div className="fixed inset-0 bg-stone-950/45 backdrop-blur-xl z-50 flex flex-col p-6 animate-in-soft">
      
      {/* Header */}
      <div className="flex items-center justify-between bg-[#fbf7ef]/95 border border-stone-200 p-4 rounded-t-[28px] neural-border">
         <div className="flex items-center gap-3">
            <div className="p-3 bg-[#f1e5d5] border border-stone-200 rounded-2xl shadow-sm">
               <Network className="w-5 h-5 text-[#9a3412]" />
            </div>
            <div>
         ...
         <div className="flex items-center gap-4">
             <div className="flex bg-white/80 border border-stone-200 rounded-2xl p-1 shadow-sm">
                <button 
                  onClick={() => setActiveTab('universe')}
                  className={`px-4 py-1.5 rounded-xl text-sm font-medium transition flex items-center gap-2 ${activeTab === 'universe' ? 'bg-[#d97757] text-white' : 'text-stone-500 hover:text-stone-900'}`}
                >
                  <Infinity className="w-4 h-4" /> Full Universe
                </button>
                <button 
                  onClick={() => setActiveTab('gallery')}
                  className={`px-4 py-1.5 rounded-xl text-sm font-medium transition flex items-center gap-2 ${activeTab === 'gallery' ? 'bg-stone-900 text-white' : 'text-stone-500 hover:text-stone-900'}`}
                >
                  <LayoutGrid className="w-4 h-4" /> Groups
                </button>
                <button 
                  onClick={() => setActiveTab('discarded')}
                  className={`px-4 py-1.5 rounded-xl text-sm font-medium transition flex items-center gap-2 ${activeTab === 'discarded' ? 'bg-[#d97757] text-white' : 'text-stone-500 hover:text-stone-900'}`}
                >
                  <Network className="w-4 h-4" /> Discarded Branches
                </button>
             </div>
             
             <button onClick={closeAnswerSpace} className="p-2 hover:bg-stone-100 rounded-2xl transition border border-transparent hover:border-stone-200">
      ...
      {/* Body Area */}
      <div className="flex-1 bg-[#f7f1e8]/95 border-x border-b border-stone-200 rounded-b-[28px] overflow-y-auto p-6">
         ...
         {activeTab === 'gallery' && (
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
               {clusters.map((cluster, i) => {
                  ...
                  return (
                  <div key={i} className={`premium-card p-5 transition-all duration-300 hover:-translate-y-1 hover:border-emerald-300/25 hover:shadow-[0_24px_80px_rgba(16,185,129,0.10)] flex flex-col ${isExpanded ? 'xl:col-span-3 min-h-[60vh]' : ''}`}>
                     <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-2">
                           <span className="px-2.5 py-1 bg-[#d97757]/10 text-[#9a3412] text-xs font-bold rounded-full border border-[#d97757]/20">
                             Cluster {i + 1}
                           </span>
                        ...
                        <div className="text-right">
                           ...
                           <div className="text-sm font-mono font-semibold text-[#9a3412]">
                     ...
                     {isComparing && compareTargetIdx === i ? (
                        <div className="mt-4 pt-4 border-t border-stone-200 flex flex-col gap-3">
                           <div className="text-[10px] text-[#9a3412] font-bold uppercase tracking-widest flex items-center gap-2">
                           ...
                           <div className="grid grid-cols-2 gap-4">
                              ...
                              <div className="p-3 bg-[#d97757]/10 border border-[#d97757]/20 rounded-xl text-[13px] text-stone-900 font-mono whitespace-pre-wrap">
                        ...
                        <div className="mt-4 pt-4 border-t border-stone-200 flex gap-2">
                           ...
                           {i !== 0 && (
                              <button 
                                 ...
                                 className="ripple-button flex-1 py-2 bg-[#d97757]/10 hover:bg-[#d97757]/15 text-[#9a3412] border border-[#d97757]/20 rounded-2xl text-xs font-semibold transition"
                              >
         ...
         {activeTab === 'discarded' && (
            <div className="flex flex-col gap-6 max-w-5xl mx-auto w-full">
               {savedBranches && savedBranches.length > 0 && (
                  <div className="premium-card p-5">
                     <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="px-3 py-1 rounded-full bg-[#d97757]/10 border border-[#d97757]/20 text-[#9a3412] text-xs font-semibold">
                          Local Research
                        </span>
                     </div>
                         ...
                         <div key={branch.id} className="rounded-2xl border border-stone-200 bg-white/75 p-3">
                           <div className="flex items-center justify-between gap-2 mb-2">
                             <div className="font-mono text-sm text-stone-900 truncate">
                               ...
                               <span className="text-[#9a3412]">{branch.alternativeToken}</span>
                             </div>
               ...
               <div className="flex flex-col items-center justify-center p-8 text-center bg-white/70 border border-stone-200 rounded-2xl">
                  <Network className="w-10 h-10 text-[#9a3412] mb-3" />
                  <h3 className="text-xl font-bold text-stone-900">Discarded Possibilities ({discardedBranches.length})</h3>
               ...
               <div className="grid grid-cols-1 gap-4">
                  {discardedBranches.map((branch, i) => {
                     ...
                     return (
                     <div key={i} className="premium-card p-5 hover:border-amber-400/30 transition-all duration-300 hover:-translate-y-0.5 flex flex-col gap-4 relative overflow-hidden group">
                        ...
                        <div className="flex justify-between items-start">
                           <div className="flex items-center gap-3">
                              ...
                              <div>
                                 ...
                                 <div className="text-sm text-stone-700">
                                    ...
                                    <span className="bg-[#d97757]/10 text-[#9a3412] font-bold px-1 rounded-sm">{branch.discardedToken}</span>
                                 </div>
                              </div>
                           </div>
                           <div className="flex flex-col items-end gap-1 shrink-0 bg-stone-100 p-2 rounded-lg border border-stone-200">
                              ...
                              <div className="flex items-center gap-3 text-xs font-mono">
                                 ...
                                 <span className="text-[#9a3412] font-bold">Discarded: {(branch.discardedProb * 100).toFixed(1)}%</span>
                              </div>
                           </div>
                        </div>
                        ...
                        <div className="pt-3 border-t border-stone-200 flex gap-3">
                           {continuationResults[i] ? (
                              <div className="w-full flex flex-col gap-2 bg-white p-4 rounded-xl border border-[#d97757]/25 shadow-inner">
                                 <div className="flex justify-between items-center mb-1 text-[11px] font-bold uppercase text-[#9a3412] tracking-wider">
                                 ...
                                 <div className="text-sm text-stone-800 leading-relaxed font-mono whitespace-pre-wrap">
                                    ...
                                    <span className="bg-[#d97757]/10 text-[#9a3412] px-1 rounded-sm font-bold">{branch.discardedToken}</span>
                              ...
                              <div className="flex gap-3 items-center w-full">
                                 <button 
                                    ...
                                    className={`px-4 py-1.5 ${continuingBranches[i] ? 'bg-stone-100 text-stone-400 cursor-not-allowed' : 'bg-[#d97757]/10 hover:bg-[#d97757]/15 text-[#9a3412]'} border border-[#d97757]/20 rounded-lg text-xs font-bold transition flex items-center gap-2`}
                                 >
